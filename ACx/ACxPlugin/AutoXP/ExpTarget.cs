    ...
    public static class ExpHelper
    {
        ...
        public static int? CostToLevel(this ExpTarget target, int offset = 0)
        {
            ...
            switch (target.TargetType())
            {
                ...
                case ExpTargetType.Attribute:
                    if (ATTRIBUTE_COSTS.Length > targetLevel + 1)
                        return ATTRIBUTE_COSTS[targetLevel];
                    break;
                case ExpTargetType.Vital:
                    if (VITAL_COSTS.Length > targetLevel + 1)
                        return VITAL_COSTS[targetLevel];
                    break;
                case ExpTargetType.Trained:
                    if (TRAINED_COSTS.Length > targetLevel + 1)
                        return TRAINED_COSTS[targetLevel];
                    break;
                case ExpTargetType.Specialized:
                    if (SPECIALIZED_COSTS.Length > targetLevel + 1)
                        return SPECIALIZED_COSTS[targetLevel];
                    break;
            }

            //Utils.WriteToChat($"Failed to get cost to level for {target.GetName()}");
            return null;
        }
        public static int GetTimesLeveled(this ExpTarget target)
        {
            switch (target.TargetType())
            {
                ...
                //    
                //    CoreManager.Current.Actions.VitalClicks
                case ExpTargetType.Vital:
            ...
            //Utils.WriteToChat($"Failed to get times leveled for {target.GetName()}");
        ...
        public static void AddExp(this ExpTarget target, int exp)
        {
            ...
            {
                case ExpTargetType.Attribute:
                    //Utils.WriteToChat($"Add experience to attribute {target}");
                    CoreManager.Current.Actions.AddAttributeExperience(target.AsAttribute(), exp);
                    break;
                case ExpTargetType.Vital:
                    //Utils.WriteToChat($"Add experience to vital {target}");
                    CoreManager.Current.Actions.AddVitalExperience(target.AsVital(), exp);
                    break;
                case ExpTargetType.Specialized:
                case ExpTargetType.Trained:
                    //Utils.WriteToChat($"Add experience to skill {target}");
                    CoreManager.Current.Actions.AddSkillExperience(target.AsSkill(), exp);
                    break;
                default:
                    Utils.WriteToChat($"Failed to add {exp} exp to {target.GetName()}");
                    break;
            }
        }
        ...
        public static ExpTargetType TargetType(this ExpTarget target)
        {
            ...
            Utils.WriteToChat($"Unable to find exp target type for {target.GetName()}");
